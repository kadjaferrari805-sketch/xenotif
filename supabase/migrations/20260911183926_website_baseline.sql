-- ============================================================================
-- Migration 20260911183926_website_baseline — Xenotif website, baseline structurelle (21 tables)
-- Dérivée du catalogue de production (pciadjwuxuevkqkdarut) lu en SELECT le 2026-09-11.
-- À appliquer UNIQUEMENT sur une base Supabase VIDE (futur projet xenotif-preview).
-- NE JAMAIS APPLIQUER À LA PRODUCTION : elle contient déjà ces objets et possède son propre
-- historique de 14 migrations. La garde ci-dessous refuse toute base non vide.
-- Structure uniquement : aucune donnée, aucun utilisateur, aucun secret, aucun bucket.
...
-- Revue : supabase/baseline/schema-baseline-review.md et schema-baseline-diff.md.
-- ============================================================================

-- GARDE — refuse une base où une table du périmètre existe déjà
do $guard$
begin
  if exists (
    select 1 from pg_catalog.pg_class c
    join pg_catalog.pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname in ('abandoned_carts', 'admin_users', 'boutique_orders', 'coach_usage', 'content_discipline_i18n', 'content_disciplines', 'content_videos', 'fitness_goals', 'health_metrics', 'newsletter_subscribers', 'profiles', 'progress', 'push_tokens', 'reviews', 'smartwatch_connections', 'smartwatch_sessions', 'stripe_events', 'subscriptions', 'user_streaks', 'web_push_subscriptions', 'workouts')
  ) then
    raise exception 'website_baseline : table du périmètre déjà présente dans public. Base non vide : migration refusée (ne jamais appliquer à la production).';
  end if;
end
$guard$;
