    ...
    <div className="rounded-2xl border border-sport-border bg-sport-card p-4 mb-6 flex items-center gap-3">
      ...
      {state === 'on' ? (
        <button
          onClick={disable}
          disabled={busy}
          className="shrink-0 inline-flex items-center gap-1.5 rounded-full border border-sport-border px-3.5 py-2 text-xs font-bold text-white hover:border-sport-orange/50 disabled:opacity-60 transition-colors"
        >
          <BellOff size={13} aria-hidden="true" /> {t('disable')}
        </button>
      ) : state === 'off' ? (
        <button
          onClick={enable}
