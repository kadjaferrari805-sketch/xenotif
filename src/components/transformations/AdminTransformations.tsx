        ...
        <div className="space-y-4">
          {items.map(it => (
            <div key={it.id} className="flex items-center gap-4 bg-sport-dark border border-sport-border rounded-xl p-3">
              ...
              <button type="button" onClick={() => moderate(it.id, 'approved')} aria-label={t('approve')} className="w-9 h-9 rounded-lg bg-emerald-50 text-[#1E7F5A] flex items-center justify-center hover:bg-emerald-500/25"><Check size={16} /></button>
