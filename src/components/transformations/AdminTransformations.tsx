        ...
        <div className="space-y-4">
          {items.map(it => (
            <div key={it.id} className="flex items-center gap-4 bg-sport-dark border border-sport-border rounded-xl p-3">
              ...
              <button type="button" onClick={() => moderate(it.id, 'approved')} aria-label={t('approve')} className="w-9 h-9 rounded-lg bg-emerald-50 text-sport-success flex items-center justify-center hover:bg-emerald-500/25"><Check size={16} /></button>
              <button type="button" onClick={() => moderate(it.id, 'rejected')} aria-label={t('reject')} className="w-9 h-9 rounded-lg bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-500/25"><X size={16} /></button>
            </div>
          ))}
        </div>
      )}
