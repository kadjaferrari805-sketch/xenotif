...
export function AdminReviews() {
  ...
  return (
    <div className="bg-sport-card border border-sport-border rounded-2xl p-6 mt-8">
      <h2 className="text-sm font-black text-white mb-4">Avis clients ({items.length})</h2>
        ...
        {items.map((r) => (
          <div key={r.id} className={`border rounded-xl p-4 ${r.hidden ? 'border-red-500/30 opacity-60' : 'border-sport-border'}`}>
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => toggle(r.id, r.hidden)} className="text-xs border border-sport-border px-3 py-1.5 rounded-full text-sport-gray hover:text-white transition-colors">
